...
export default async function PlacesPage({ params }: { params: Promise<{ tripId: string }> }) {
  ...
  const [{ role }, places, logistics] = await Promise.all([
    getTripContext(tripId),
    getPlaces(tripId, true),
    getTripLogistics(tripId),
  ]);
  const editable = canEdit(role);
  const activePlaces = places.filter((place) => !place.deleted_at);
  const savedCount = activePlaces.length + logistics.hotels.length + logistics.flights.length;
            ...
            {logistics.hotels.map((hotel) => (
              <Card key={hotel.id}>
                <CardContent className="flex h-full flex-col p-5">
                  ...
                  {editable ? <form className="mt-auto pt-5" action={softDeleteTripHotel.bind(null, tripId, hotel.id)}><Button type="submit" variant="ghost" className="text-[var(--danger)]"><Trash2 size={16} />Delete</Button></form> : null}
                </CardContent>
              </Card>
            ))}
          </div>
            ...
            {logistics.flights.map((flight) => (
              <Card key={flight.id}>
                <CardContent className="flex h-full flex-col p-5">
                  ...
                  {editable ? <form className="mt-auto pt-5" action={softDeleteTripFlight.bind(null, tripId, flight.id)}><Button type="submit" variant="ghost" className="text-[var(--danger)]"><Trash2 size={16} />Delete</Button></form> : null}
                </CardContent>
              </Card>
            ))}
          </div>
              ...
              <Card key={place.id} className={place.deleted_at ? "opacity-60" : undefined}>
                <CardContent className="flex h-full flex-col p-5">
                  ...
                  <div className="mt-auto flex flex-wrap gap-2 pt-5">
                    ...
                    {editable && !place.deleted_at ? <form action={softDeletePlace.bind(null, tripId, place.id)}><Button variant="ghost" type="submit" className="text-[var(--danger)]"><Trash2 size={16} />Delete</Button></form> : null}
